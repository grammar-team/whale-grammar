...
const INSPECTION_LISTENER = {
  _filterErrorWords: (html) => {
    ...
    wordEls.forEach((wordEl) => {
      ...
      if (innerText) words.push(`${innerText}`);
    });
...
whale.runtime.onConnect.addListener(function (port) {
  if (port.name !== `grammar-inspection`) return;
...
// whale.runtime.onInstalled.addListener(function ({ reason }) {
//   whale.storage.sync.get(["show_notion"], function ({ show_notion }) {
//     if (reason === `install` && show_notion === undefined) {
//       whale.storage.sync.set({ show_notion: true });
//       whale.tabs.create({
